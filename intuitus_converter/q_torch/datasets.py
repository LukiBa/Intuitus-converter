...
class LoadImagesAndLabels_FaceDescriptor(LoadImagesAndLabels2):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, image_weights=False,
                 cache_images=False, single_cls=False):
        path = Path(path).absolute()  # os-agnostic
        assert os.path.isfile(path), 'File not found %s. See %s' % (path, help_url)
        ...
        with open(path, 'r') as annotation_file:
            ...
            for num, line in enumerate(pbar):
                ...
                if bbox_data_gt.shape[0]:
                    self.labels.append(bbox_data_gt) # change position from class to 0            
                    nf += 1  # file found
                else:
                    ne += 1  # print('empty labels for image %s' % self.img_files[i])  # file empty
    ...
    def __getitem__(self, index):
        ...
        if x.size > 0:
            # Normalized xywh to pixel xyxy format
            labels = x.copy()
            labels[:, 0] = ratio[0] * w/w0 * x[:, 0] + pad[0]  # pad width
            labels[:, 1] = ratio[1] * h/h0 * x[:, 1] + pad[1]  # pad height
            labels[:, 2] = ratio[0] * w/w0 * x[:, 2] + pad[0]
            labels[:, 3] = ratio[1] * h/h0 * x[:, 3] + pad[1]
        ...
        if nL:
            ...
            # Normalize coordinates 0 - 1
            labels = labels.astype(np.float32)
            labels[:, [1, 3]] = labels[:, [1, 3]] / img.shape[0]  # height
            labels[:, [0, 2]] = np.float32(labels[:, [0, 2]] / img.shape[1])  # width

        labels_out = torch.zeros((nL, 133))
        if nL:
            labels_out[:, 1:] = torch.from_numpy(labels)

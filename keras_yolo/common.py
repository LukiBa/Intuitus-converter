...
from intuitus_converter.q_keras import Conv2D_int8
...
def convolutional(id,input_layer, filters_shape, downsample=False, activate=True, bn=True, quant = False, allow_immediate_maxpool = True, activate_type='leaky'):
    if downsample:
        #input_layer = tf.keras.layers.ZeroPadding2D(((1, 0), (1, 0)))(input_layer) + padding top+left
        #padding = 'valid'
        padding = 'same' # padding bottom+right
        strides = 2
    else:
        strides = 1
        padding = 'same'
    ...
    if quant:
        ...
        if bn or not activate:
            conv = convolution_class(filters=filters_shape[-1], kernel_size = filters_shape[0], strides=strides,
                                     padding=padding, use_bias=not bn, allow_immediate_maxpool = allow_immediate_maxpool,
                                     kernel_regularizer=tf.keras.regularizers.l2(0.0005),
                                     kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                     bias_initializer=tf.constant_initializer(0.),name=name)(input_layer)
        else:
            conv = convolution_class(filters=filters_shape[-1], kernel_size = filters_shape[0], strides=strides,
                                     padding=padding, use_bias=not bn, activation='relu', allow_immediate_maxpool = allow_immediate_maxpool,
                                     kernel_regularizer=tf.keras.regularizers.l2(0.0005),
                                     kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                     bias_initializer=tf.constant_initializer(0.),name=name)(input_layer)        

    ...
    if not fbn:
        ...
        if is_tiny:
            if model == 'yolov4':
                ...
                return YOLOv3_tiny_fbn(input_layer, NUM_CLASS)
        else:
            if model == 'yolov4':
...
def YOLOv4_tiny(input_layer, NUM_CLASS):
    ...
    conv = convolutional(15,conv, (1, 1, 512, 256))

    conv_lobj_branch = convolutional(16,conv, (3, 3, 256, 512))
    conv_lbbox = convolutional(17,conv_lobj_branch, (1, 1, 512, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    conv = convolutional(18,conv, (1, 1, 256, 128))
    conv = upsample(conv)
    conv = tf.concat([conv, route_1], axis=-1)

    conv_mobj_branch = convolutional(19,conv, (3, 3, 128, 256))
    conv_mbbox = convolutional(20,conv_mobj_branch, (1, 1, 256, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    return [conv_mbbox, conv_lbbox]

def YOLOv4_tiny_fbn(input_layer, NUM_CLASS):
    route_1, conv = cspdarknet53_tiny_folding_bn(input_layer)

    conv = convolutional(15,conv, (1, 1, 512, 256), bn=False)

    conv_lobj_branch = convolutional(16,conv, (3, 3, 256, 512), bn=False)
    conv_lbbox = convolutional(17,conv_lobj_branch, (1, 1, 512, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    conv = convolutional(18,conv, (1, 1, 256, 128), bn=False)
    conv = upsample(conv)
    conv = tf.concat([conv, route_1], axis=-1)

    conv_mobj_branch = convolutional(19,conv, (3, 3, 128, 256), bn=False)
    conv_mbbox = convolutional(20,conv_mobj_branch, (1, 1, 256, 3 * (NUM_CLASS + 5)), activate=False, bn=False)
...
def YOLOv3_tiny(input_layer, NUM_CLASS):
    ...
    conv = convolutional(7,conv, (1, 1, 1024, 256))

    conv_lobj_branch = convolutional(8,conv, (3, 3, 256, 512))
    conv_lbbox = convolutional(9,conv_lobj_branch, (1, 1, 512, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    conv = convolutional(10,conv, (1, 1, 256, 128))
    conv = upsample(conv)
    conv = tf.concat([conv, route_1], axis=-1)

    conv_mobj_branch = convolutional(11,conv, (3, 3, 128, 256))
    conv_mbbox = convolutional(12,conv_mobj_branch, (1, 1, 256, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    return [conv_mbbox, conv_lbbox]

def YOLOv3_tiny_fbn(input_layer, NUM_CLASS):
    route_1, conv = darknet53_tiny_folding_bn(input_layer)

    conv = convolutional(7,conv, (1, 1, 1024, 256), bn=False, quant = True)

    conv_lobj_branch = convolutional(8,conv, (3, 3, 256, 512), bn=False, quant = True)
    conv_lbbox = convolutional(9,conv_lobj_branch, (1, 1, 512, 3 * (NUM_CLASS + 5)), 
                                      activate=False, bn=False, quant = True)

    conv = convolutional(10,conv, (1, 1, 256, 128), bn=False, quant = True)
    conv = upsample(conv)
    conv = tf.concat([conv, route_1], axis=-1)

    conv_mobj_branch = convolutional(11,conv, (3, 3, 128, 256), bn=False, quant = True)
    conv_mbbox = convolutional(12,conv_mobj_branch, (1, 1, 256, 3 * (NUM_CLASS + 5)), 
                                      activate=False, bn=False, quant = True)

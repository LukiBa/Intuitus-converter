...
from torch_yolo.models import *  # set ONNX_EXPORT in models.py
from intuitus_converter.q_torch.datasets import *
from intuitus_converter.q_torch.utils import *
import numpy as np
import torch

def _create_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument('--cfg', type=str, default='torch_yolo/cfg/yolov3tiny/yolov3-tiny-quant.cfg', help='cfg file path')
    parser.add_argument('--weights', type=str, default='torch_yolo/weights/rt_best.pt', help='path to weights file')
    parser.add_argument('--img_size', type=int, default=416, help='inference size (pixels)')
    parser.add_argument('--quantized', type=int, default=1,help='qunatization method. 0:float32, 1:int8, 2:int8 post_scale')
    parser.add_argument('--param_outpath', type=str, default='./parameters/int8_6', help='Path to output parameter. If set to None parameter are not saved.') #'./detect_imgs'
    parser.add_argument('--out_weights', type=str, default='torch_yolo/weights/post_scale.pt', help='weights path')
    ...
    parser.add_argument('--accum_bits', type=int, default=15, help='accum-bit. MAC width')
    return parser.parse_args()    

def export_parameter_npy(model,path,export_quantized=False,w_bits=6,b_bits=6,a_bits=8,accum_bits=15):
    b_min = -(1 << (b_bits - 1))
    b_max = (1 << (b_bits - 1)) - 1
    w_min = -(1 << (w_bits - 1))+1
    w_max = (1 << (w_bits - 1))-1 
    ...
    if not export_quantized:
        ...
        for key in parameters:                              
            ...
            if '.weight' in key and not 'shift' in key:
                ...
                bias = parameters[key] << parameters[layer_name + '.bias_quant_shift']
                ...
                print(layer_name + ' activation_shift:' + str(out_shift))

            elif '.bias_quant_shift' in key:
                bias_shift = (parameters[layer_name + '.bias_shift']+parameters[layer_name + '.weight_shift']-parameters[key])   
                bias_shift = accum_bits-b_bits-bias_shift.data.numpy()
                #print(layer_name + ' bias_shift:' + str(bias_shift))
                np.save(str(path/key),bias_shift)
...
def convert(opt):
    ...
    post_scale_model = Darknet(opt.cfg, img_size, quantized=2)
    ...
    for key in parameters:
        if 'scale' in key:
            ...
            if'weight_quantizer.scale' in key:
                ...
                if layer_nbr == 0:
                    ...
                    if layer_nbr in output_layers:
                        ...
                        print(rshift_name+".shift" + str(new_parameters[rshift_name+".shift"]))
                    else:
                        new_parameters[layer_name+".activation_shift"].copy_(torch.tensor(scale[0]))
            ...
            elif 'bias_quantizer.scale' in key:
                new_parameters[layer_name+".bias_quant_shift"].copy_(torch.tensor(scale[0]))
            else:
    ...
    post_scale_model.to(device).eval() # Eval mode
    
    if opt.param_outpath != None:
        param_outpath = pathlib.Path(opt.param_outpath)
        if param_outpath.exists():
            shutil.rmtree(param_outpath, ignore_errors=False, onerror=None)    
        param_outpath.mkdir(parents=True, exist_ok=True)

        export_parameter_npy(post_scale_model,opt.param_outpath,export_quantized=True,
                             w_bits=opt.w_bits,b_bits=opt.b_bits,a_bits=opt.a_bits,accum_bits=opt.accum_bits)

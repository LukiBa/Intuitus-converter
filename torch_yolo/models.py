...
import intuitus_converter.q_torch.quantized_intuitus_int as q_int
...
def create_modules(module_defs, img_size, cfg, quantized, quantizer_output, a_bit=8, w_bit=8, 
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if quantized == 1:
                modules.add_module('Conv2d', q_int.QuantizedConv2d_WeightOnly(in_channels=output_filters[-1],
                ...
                #    modules.add_module('result_scale', q_int.IntuitusReslult_shift())#q_int.IntuitusResult_Linear(bias=False))              
            elif quantized == 2:
                if FPGA:
                    modules.add_module('Conv2d', q_int.QuantizedConv2d_fpga(in_channels=output_filters[-1],
                                                                            out_channels=filters,
                                                                            kernel_size=kernel_size,
                                                                            stride=int(mdef['stride']),
                                                                            padding=pad,
                    ...
                    modules.add_module('Conv2d', q_int.QuantizedConv2d(in_channels=output_filters[-1],
                                                                       out_channels=filters,
                                                                       kernel_size=kernel_size,
                                                                       stride=int(mdef['stride']),
                                                                       padding=pad,
                ...
                if FPGA:
                    modules.add_module('Conv2d', q_int.QuantizedConv2d_post_shift_fpga(in_channels=output_filters[-1],
                                                                            out_channels=filters,
                                                                            kernel_size=kernel_size,
                                                                            stride=int(mdef['stride']),
                                                                            padding=pad,
                    ...
                    modules.add_module('Conv2d', q_int.QuantizedConv2d_post_shift(in_channels=output_filters[-1],
                                                                       out_channels=filters,
                                                                       kernel_size=kernel_size,
                                                                       stride=int(mdef['stride']),
                                                                       padding=pad,
                ...
                if mdef['activation'] == 'mish':
                    modules.add_module('activation', Mish())     
                if mdef['activation'] == 'linear':
                    modules.add_module('result_shift', q_int.IntuitusReslult_shift())#q_int.IntuitusResult_Linear(bias=False))   
            else:
            ...
            pad = (kernel_size - 1) // 2 if int(mdef['pad']) else 0
            if quantized == 1:
                raise NotImplementedError("Quantized depthwise conv not implemented yet")
            elif quantized == 2:
                raise NotImplementedError("Quantized post scale depthwise conv not implemented yet")
            else:
...
def load_darknet_weights(self, weights, cutoff=-1, pt=False, FPGA=False):
    ...
    file = pathlib.Path(weights).name
    ...
    # Ensure that the pointer reaches the last position of the weight
